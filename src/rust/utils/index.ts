...
export function shouldIncludeHostCall(context: Context): boolean {
  let interfaces = context.config.hostInterfaces as Array<String>;
  return shouldInclude(context, interfaces);
}

export function shouldIncludeHandler(context: Context): boolean {
  let interfaces = context.config.handlerInterfaces as Array<String>;
  return shouldInclude(context, interfaces);
}

function shouldInclude(context: Context, interfaces: Array<String>): boolean {
  if (context.interface != undefined) {
    if (
      interfaces == undefined ||
      interfaces.indexOf(context.interface.name) == -1
    ) {
      return false;
    }
